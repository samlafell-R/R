...
ggplot(data = sfo_feb_flights, aes(x=arr_delay)) +
  geom_histogram()
sfo_feb_flights %>%
  summarise(mean_arr_delay = mean(arr_delay), median_arr_delay = median(arr_delay))
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(IQR_arr_delay = IQR(arr_delay), median(arr_delay))
```
...
nycflights %>%
  summarise(ot_arr_w_delayed_dep = sum(arr_type == "on time") / sum(dep_type == 'delayed'))
```
